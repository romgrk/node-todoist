...
import { State, TodoistResources, TodoistResponse } from './types'
...
export const Todoist = (token: string, userOptions = defaultOptions) => {
  ...
  const executeCommand = async (type: keyof TodoistResources, action: string, args: {} = {}) => {
    ...
    const sync_status = res.sync_status[id]

    if (sync_status !== 'ok') {
      const error = new Error(`${sync_status.error_tag}: ${sync_status.error}`)
